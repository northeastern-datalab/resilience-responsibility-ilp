...
from src.utils import addTuples, pickRandomResponsibilityTuple, performSemijoinReduction

CREATE_DATA = True
OVERWRITE_DATA = False
DATA_SAVE_FILE = 'data/resp_synthetic_expt/expt-data-case-{}.csv'
DATA_INSTANCE_PICKLE_FOLDER = 'data/data_instances_pickle_dump/'
EXPT_CASE_DETAILS_FILE = 'expt/synthetic/synthetic_resp_data_expt_cases.csv'
...
def runCase(case_no):
    ...
    runTestCase(case_no, case_details['query_name'], case_details['resp_table'], case_details['domain_size'] , case_details['itr_log_base'], \
...
def runTestCase(case_no, query_name, resp_table, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10):
    """ 
    ...
    """
    if CREATE_DATA:
        runMonotoneRunOfExperiments(query_name, resp_table, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = bag_semantics, max_bag_size = max_bag_size, output_filename=DATA_SAVE_FILE.format(case_no), overwrite_data = OVERWRITE_DATA)
        print('Experiment Data Generation complete. Data is stored in', DATA_SAVE_FILE.format(case_no))
    
    


def runMonotoneRunOfExperiments(query_name, resp_table, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False):
    """
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        if resp_tuple is not None:
            ...
            if len(expt_results["lp_results"]) == 0 or expt_results["lp_results"]["Responsibility"] == 0:
            ...
            if query_name == "3Star" or query_name == "Triangle" or (query_name == "TriangleUnary" and resp_table != "A"):
                ...
                expt_results["flow_witness_linearization_networkx_results"] = responsibility_with_flow_nonlinear_query(query, pruned_database_instance, resp_tuple, tuple_weights = tuple_weights, method = "NetworkX", linearization_method = "constant_witness", ptime_linearization = True)
                expt_results["flow_witness_linearization_lp_results"] = responsibility_with_flow_nonlinear_query(query, pruned_database_instance, resp_tuple, tuple_weights = tuple_weights, method = "LP", linearization_method = "constant_witness", ptime_linearization = True)
