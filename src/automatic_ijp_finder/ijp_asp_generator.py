...
def generate_asp_program(query_name, endpoint_table, max_domain, optimize = True, check_composability = True, PROGRAM_SAVE_FOLDER=''):
    '''
    For a given query generate the asp problem to check if there is an ijp or not
    '''
    query = queries[query_name]
    file_key = query_name+'-'+endpoint_table+'-'+str(max_domain)
    # file_key = case_no
    f = open(PROGRAM_SAVE_FOLDER+file_key+".dl", "w") 
...
def main(args): 
    if len(args) == 2:
        parser = argparse.ArgumentParser(description='Generate an ASP program to find if there exists an IJP of a given domain size for a query')
        ...
        parser.add_argument('endpoint', type=str, help="The name of the relation to be used as the endpoint")
        parser.add_argument('domain_size', type=int, help="The max domain size of the ijp to be generated")
        parser.add_argument('--program_save_folder', type=str, help="The folder where the generated asp program should be saved")
        args = parser.parse_args()
        generate_asp_program(args.query, args.endpoint, args.domain_size, PROGRAM_SAVE_FOLDER = args.program_save_folder)
