...
from src.utils import performSemijoinReduction, computeWitnesses
from expt.tpch.tpch_data_details import tpch_query_tables, tpch_schema

CREATE_DATA = True
OVERWRITE_DATA = False
DATA_SAVE_FILE = 'data/tpch_expt/expt-data-case-{}.csv'
EXPT_CASE_DETAILS_FILE = 'expt/tpch/tpch_data_expt_cases.csv'
TPCH_DATA_FOLDER = '/work/wolfandthegang/data/tpch/tpch-sf{}/'
TPCH_PICKLE_FILE = '/work/wolfandthegang/data/tpch/tpch-sf{}/query-{}-pruned-database-instance.pkl'
...
def runCase(case_no):
    ...
    runTestCase(case_no, case_details['query_name'], case_details['scale_factors'], bag_semantics = case_details['bag_semantics'])


def runTestCase(case_no, query_name, sf_factors, bag_semantics = False):
    """ 
    ...
    """
    if CREATE_DATA:
        runExperiment(query_name, sf_factors, bag_semantics = bag_semantics, output_filename=DATA_SAVE_FILE.format(case_no), overwrite_data = OVERWRITE_DATA)
        print('Experiment Data Generation complete. Data is stored in', DATA_SAVE_FILE.format(case_no))

def runExperiment(query_name, scale_factors, bag_semantics = False, output_filename='data/tpch_expt/unknown_query_data.csv', overwrite_data = False, time_limit = 1200):
    """
    ...
    for sf in scale_factors.split():
        ...
        if not os.path.exists(output_filename) or overwrite_data:
            ...
            with open(output_filename, 'w', newline='') as csv_file:
                ...
                writer.writeheader()
                output_format = {i:output[i] if i in output else None for i in expt_headers}
                writer.writerow(output_format)
                overwrite_data = False
        else:            
            if len(expt_headers) == 0:
                ...
                with open(output_filename, 'r') as csv_file:
                    ...
                    expt_headers = d_reader.fieldnames
